...
"""
╔══════════════════════════════════════════════════════════════╗
║   HỆ THỐNG PHÂN LOẠI RÁC THÔNG MINH v3.0 (REVOLUTION)        ║
║   Smart Trash Classification System                          ║
║                                                              ║
║   AI Engine : YOLO11m (Ultralytics) + Object Tracking        ║
║   GUI       : CustomTkinter                                  ║
║   Hardware  : ESP32-S3 EYE (Camera) + ESP32-S3 (Controller)  ║
...
class Theme:
    # ECO Nature Theme - Light Lime & Forest Green
...
class TrashClassifierApp(ctk.CTk):
    ...
    def __init__(self):
        ...
        # ---- Window ----
        self.title("♻️ Hệ Thống Phân Loại Rác Thông Minh v5.0")
        ...
        self._http_queue: Queue = Queue() # Hàng đợi Async HTTP
        self.tracked_ids = set() # Lưu các Object ID đã phân loại
        ...
        # ---- Boot ----
        self._log("ℹ️", "Hệ thống v4.0 khởi động (Auto-Discovery)...", Theme.INFO)
    ...
    def _run_auto_discovery(self):
        def _scan():
            ...
            if not found_cam and not found_ctrl:
                self._log("⚠️", "Không tìm thiết bị mới nào, dùng IP cũ.", Theme.WARNING)
    ...
    def _build_header(self):
        ...
        ctk.CTkLabel(left, text="♻️  HỆ THỐNG PHÂN LOẠI RÁC THÔNG MINH",
    ...
    def _build_video_card(self, parent):
        ...
        # Container cố định cho video — ngăn không cho label phóng to đè lên layout
    ...
    def _build_sidebar(self, parent):
        # Dùng lại ScrollableFrame để an toàn trên mọi độ phân giải màn hình
        sb = ctk.CTkScrollableFrame(parent, fg_color="transparent", width=380)
        sb.grid(row=0, column=1, rowspan=2, sticky="nsew")

        self._card_model(sb)      # Đưa lên đầu để dễ thấy nhất
    ...
    def _card_controls(self, p):
        c = ctk.CTkFrame(p, fg_color=Theme.BG_CARD, corner_radius=12,
                         border_width=1, border_color=Theme.BORDER)
        c.pack(fill="x", pady=(0, 6))
        ctk.CTkLabel(c, text="⚙️  BẢNG ĐIỀU KHIỂN", font=Theme.HEADING,
                     text_color=Theme.ACCENT, anchor="w").pack(fill="x", padx=12, pady=(8, 4))
    ...
    def _select_model(self):
        ...
        if file_path:
            ...
            self._log("ℹ️", f"Đã chọn model mới: {os.path.basename(file_path)}", Theme.INFO)
            self._load_model()
    ...
    def _start(self):
        ...
        self.video = VideoStream(self.cfg["camera_stream_url"])
        self.video.start()
        self._log("ℹ️", "Đang kết nối camera...", Theme.INFO)
    ...
    def _stop(self):
        ...
        self._log("ℹ️", "Hệ thống đã dừng.", Theme.INFO)

    # ---- Vòng lặp Nhận diện (YOLO Tracking) ----
                ...
                try:
                    ...
                    if results[0].boxes.id is not None:
                        ...
                        for i in range(len(boxes)):
                            ...
                            # CÁCH MẠNG: Chỉ ra lệnh nếu Object ID này chưa từng được đếm
        ...
        while self.is_running:
            try:
                ...
                if result.get("status") == "error":
                    ...
                    self._log("✅", f"Lệnh được ESP32 tiếp nhận xử lý ngầm ({rt:.3f}s)", Theme.SUCCESS)
                    ...
                    self.stats[cid] += 1
                    self.total += 1
                    self.after(0, lambda: self.lbl_det_status.configure(text=f"✅ Đã thêm vào hàng đợi ESP32", text_color=Theme.SUCCESS))
    ...
    def _push_firebase(self):
        ...
        def _do_push():
            try:
                ...
                self._log("☁️", "Đã đồng bộ lên Firebase Cloud!", Theme.INFO)
            except Exception as e:
    ...
    def _reset_stats(self):
        ...
        # 2. Xóa dữ liệu trong Database (SQLite) để không bị cộng dồn khi khởi động lại
        try:
            self.cursor.execute("DELETE FROM sort_log")
            self.conn.commit()
            self._log("🗑️", "Đã xóa lịch sử trong Database.", Theme.INFO)
        except Exception as db_e:
            self._log("❌", f"Lỗi xóa Database: {db_e}", Theme.DANGER)

        # 3. Cập nhật giao diện và Cloud
        self._refresh_stats()
        self._push_firebase()
    ...
    def _on_close(self):
        self._log("ℹ️", "Đang tắt hệ thống...", Theme.INFO)
        self.is_running = False
