/*
 * ╔══════════════════════════════════════════════════════════════╗
 * ║   HỆ THỐNG PHÂN LOẠI RÁC THÔNG MINH - MODULE CƠ CẤU (V3.0)   ║
 * ║   Board: ESP32-S3 | Firmware v3.0 (CÁCH MẠNG KHÔNG CHẶN)     ║
 * ╚══════════════════════════════════════════════════════════════╝
 */
...
const char* classNames[4] = {"Chai nuoc", "Lon nuoc ngot", "Vo keo", "Thuoc la"};
int sortCycleCounter = 0; // Đếm số lần sort để auto-home
...
void handleSort() {
  ...
  // Khởi động State Machine (Non-blocking)
  ...
  // Trả về ngay lập tức HTTP 202 Accepted
    ...
    case STATE_SORT_SERVO_CLOSING:
      if (millis() - stateTimer >= 10) { // Cứ 10ms đóng 1 độ (Smooth Close)
        ...
        if (servoAngle <= SERVO_CLOSED) {
          // Loại bỏ lệnh stepper.moveTo(0) để đĩa không quay về điểm gốc sau mỗi lần thả
          currentState = STATE_SORT_MOVING_TO_HOME;
        }
      }
      break;

    case STATE_SORT_MOVING_TO_HOME:
      stepper.run();
      if (stepper.distanceToGo() == 0) {
        totalSorted++;
...
void setup() {
  ...
  while (digitalRead(hallPin) == HIGH && millis() < timeout) {
    stepper.runSpeed();
  }
  stepper.setCurrentPosition(0);
  enableStepper(false);
  Serial.println("[OK] San sang!");
