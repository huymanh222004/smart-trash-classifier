/*
 * ╔══════════════════════════════════════════════════════════════╗
 * ║   HỆ THỐNG PHÂN LOẠI RÁC THÔNG MINH - MODULE CAMERA       ║
 * ║   Board: ESP32-S3 EYE | Firmware v2.1                      ║
 * ╚══════════════════════════════════════════════════════════════╝
 *
 * TÍNH NĂNG MỚI v2.1 — WiFi Manager:
 *   - Lần đầu chạy (hoặc đổi mạng): ESP32 tạo hotspot
 *     "SmartTrash-CAM" → kết nối bằng điện thoại → vào 192.168.4.1
 *     → nhập SSID + Password WiFi mới → ESP32 tự lưu & kết nối
 *   - Thông tin WiFi lưu vĩnh viễn trên flash, giữ nguyên khi tắt nguồn
 *   - Muốn đổi mạng: gửi "RESET" qua Serial Monitor → ESP32 xóa
 *     WiFi cũ và tạo lại hotspot để cấu hình mạng mới
 */
...
// Trang HTML cấu hình WiFi — giao diện đẹp, dễ dùng trên điện thoại
...
// Trang thông báo đã lưu thành công
...
void handleConfigSave() {
  ...
  if (ssid.length() > 0) {
    ...
    // Đảm bảo gửi xong web mới restart
...
// Khởi chạy AP mode + trang cấu hình
void startConfigPortal() {
  ...
  // Chờ người dùng cấu hình (vòng lặp vô hạn cho đến khi restart)
  while (true) {
    configServer.handleClient();
    delay(10);

    // Kiểm tra Serial — gửi "RESET" để xóa WiFi
    if (Serial.available()) {
...
void loop() {
  ...
  // Xử lý UDP Broadcast Auto-Discovery
